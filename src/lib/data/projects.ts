...
const items: Array<Project> = [
	{
		...
		color: '#5e95e3',
		description:
			'This a deep learning system for detecting and segmenting intracranial cerebral aneurysms in Time-of-Flight Magnetic Resonance Angiography (TOF-MRA). We used a 3D U-Net architecture and incorporated an intelligent patch selection strategy, guided by neuro-vascular anatomy, to help the model focus on the most high-probabilty regions for aneurysm occurence, significantly improving efficiency and accuracy. Similarly, during inference, we used an anatomically-informed sliding window approach. Ground truth labelling was performed by certified neuroradiologists, but instead of using voxel-wise annotations, they generated \'weak\', speherical labels that encompassed the aneurysm and were much faster to generate. Due to GPU constraints, we had to optimize the pipeline\'s memory usage—reducing the training footprint by over 40%—through custom TensorFlow augmentation functions and efficient data loading. After determining the optimal network configuration and training, we were able to achieve a 0.76 Dice score and 83% sensitivity on the test set, demonstrating the model\'s effectiveness in identifying aneurysms in complex 3D MRA volumes.',
		shortDescription:
		...
		screenshots: [
			{
				...
				src: `${base}/screenshots/synack.png`,
			},
			{
				label: 'Example Test Case 2',
				src: `${base}/screenshots/segcontrol.png`
			}
	...
	},
	{
		...
		screenshots: [
			...
			{
				label: 'Model Accuracies',
				src: `${base}/screenshots/ram_accuracies.png`
			},
		...
		screenshots: [
			{
				...
				src: `${base}/screenshots/userflow.png`
			},
			{
				label: 'Endpoints',
				src: `${base}/screenshots/endpoints.png`
			},
			{
